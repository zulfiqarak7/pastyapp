...
import { ShoppingBag, X, Music, ExternalLink, ChevronRight, Check, ArrowRight } from 'lucide-react';

/**
 * PA$TY OFFICIAL WEBSITE & STORE
 * * Instructions for User:
 * 1. Ensure 'logo.png' and 'background.jpg' are inside your 'public' folder.
 * 2. This code automatically uses those files.
 */
...
// 1. Custom Cursor Component ($ Sign - Sporty Version)
const CustomCursor = () => {
  ...
  useEffect(() => {
    ...
    // Check for hover over clickable elements to change cursor state
  ...
  return (
    ...
    >
      <div className={`relative flex items-center justify-center transition-all duration-300 ${isHovering ? 'scale-150 -rotate-12' : 'scale-100'}`}>
         {/* Sporty Dollar Sign Cursor (Sans-Serif, Black, Italic) */}
         <span className="text-green-500 text-5xl font-sans italic font-black tracking-tighter drop-shadow-[2px_2px_0px_rgba(0,0,0,1)]">
           $
         </span>
      </div>
    </motion.div>
...
// 2. Glitch Text Component (For Nav)
const GlitchLink = ({ text, onClick }) => {
...
// 3. Infinite Marquee Component (Sporty Pattern)
const Marquee = () => {
  return (
    <div className="bg-green-500 text-black overflow-hidden py-3 border-y border-black relative z-40 select-none">
      <div className="flex whitespace-nowrap">
        <motion.div 
          className="flex items-center text-xl font-black uppercase tracking-widest"
          animate={{ x: [0, -1035] }} // Adjust based on content width roughly
          transition={{ repeat: Infinity, ease: "linear", duration: 10 }}
        >
          {/* Duplicated content for seamless loop */}
          {[...Array(10)].map((_, i) => (
             <span key={i} className="mx-6 text-2xl italic font-black flex items-center gap-4">
                /// PA$TY
             </span>
          ))}
        </motion.div>
...
export default function App() {
  ...
  return (
    ...
    <div className="bg-black text-white min-h-screen font-sans selection:bg-green-500 selection:text-black overflow-x-hidden cursor-none">
      ...
      <section className="relative h-[120vh] w-full overflow-hidden">
        <div className="fixed top-0 left-0 w-full h-full z-0 flex justify-center items-end pointer-events-none">
           <motion.img 
            ...
            alt="Pa$ty" 
            className="w-full h-[90vh] md:h-[100vh] object-cover object-center"
          />
          <div 
             className={`absolute inset-0 bg-white z-[5] pointer-events-none mix-blend-overlay transition-opacity duration-100 ease-out ${showLightning ? 'opacity-60' : 'opacity-0'}`}
          />
          <motion.div 
      ...
      <div className="relative z-30 bg-black w-full border-t border-gray-900 rounded-t-[3rem] shadow-[0_-20px_50px_rgba(0,0,0,1)] mt-[-20vh] overflow-hidden">
        ...
        <section id="music" className="py-24 px-6 md:px-12 max-w-7xl mx-auto">
          <motion.div
            ...
            viewport={{ once: true }}
            className="mb-16 border-b border-gray-800 pb-8 flex justify-between items-end"
          >
             <div>
              <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter text-white">The Sound</h2>
              <p className="text-gray-400 mt-2">Latest visuals & releases.</p>
             </div>
          ...
          <div className="grid md:grid-cols-2 gap-12">
            <motion.div 
              ...
              viewport={{ once: true }}
              className="relative aspect-video bg-gray-900 rounded-xl overflow-hidden border border-gray-800 shadow-2xl shadow-green-900/10"
            >
              <iframe 
            ...
            >
              {TRACKS.map((track, i) => (
                <a 
                  ...
                  className="flex items-center justify-between p-4 bg-gray-900/50 border border-gray-800 rounded-lg hover:bg-gray-800 hover:border-green-500 transition-all cursor-pointer group"
                >
                  <div className="flex items-center gap-4">
                    <span className="text-gray-500 text-sm font-mono">0{i+1}</span>
                    <div>
                      <h4 className="font-bold group-hover:text-green-400 transition-colors">{track.title}</h4>
                      <p className="text-xs text-gray-400">Apple Music</p>
                    </div>
                  </div>
                  <span className="text-sm text-gray-500 flex items-center gap-2">
                    {track.length} <ExternalLink size={12} />
                  </span>
                </a>
              ))}
              <a 
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full py-4 mt-4 border border-green-500 text-green-500 uppercase font-bold tracking-widest hover:bg-green-500 hover:text-black transition-colors rounded flex items-center justify-center gap-2"
              >
                  ...
                  <div className="relative aspect-[4/5] bg-gray-800 rounded-2xl overflow-hidden mb-4">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                       <button 
                        ...
                        className="w-full bg-white text-black py-3 font-bold uppercase tracking-wide hover:bg-green-500 transition-colors"
                       >
                         Add to Cart
                       </button>
                    </div>
               ...
               <div className="flex flex-col md:flex-row gap-4">
                 <div className="flex-1 relative group">
                   ...
                   <input 
                      ...
                      className="relative w-full bg-black border border-gray-800 text-white p-4 uppercase font-mono tracking-widest placeholder:text-gray-600 focus:outline-none focus:border-green-500 transition-colors"
                   />
                 </div>
                 <button className="bg-green-500 text-black px-8 py-4 font-bold uppercase tracking-widest hover:bg-green-400 transition-colors flex items-center justify-center gap-2">
        ...
        {isCartOpen && (
          <>
            <motion.div 
              ...
              className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 cursor-crosshair"
            />
            <motion.div 
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              exit={{ x: '100%' }}
              className="fixed top-0 right-0 h-full w-full md:w-[400px] bg-gray-900 border-l border-gray-800 z-50 flex flex-col"
            >
              <div className="p-6 border-b border-gray-800 flex justify-between items-center">
                <h2 className="text-xl font-bold uppercase">Your Cart ({cart.length})</h2>
                <button onClick={() => setIsCartOpen(false)} className="hover:text-red-500">
                  <X />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                {cart.length === 0 ? (
                  <div className="text-center text-gray-500 mt-10">
                    ...
                    <button onClick={() => setIsCartOpen(false)} className="mt-4 text-green-500 underline">Start Shopping</button>
                  </div>
                ) : (
                  cart.map((item, idx) => (
                    <div key={idx} className="flex gap-4 items-center bg-black/40 p-3 rounded-lg">
                      <img src={item.image} className="w-16 h-16 object-cover rounded" alt="item" />
                      <div className="flex-1">
                        <h4 className="font-bold text-sm line-clamp-1">{item.name}</h4>
                        <p className="text-green-400 text-sm font-mono">${item.price}</p>
                      </div>
              ...
              {cart.length > 0 && (
                <div className="p-6 border-t border-gray-800 bg-black">
                  ...
                  <button className="w-full bg-green-500 text-black font-bold uppercase py-4 rounded tracking-widest hover:bg-green-400 transition-colors flex justify-center items-center gap-2">
      ...
      <AnimatePresence>
        {notification && (
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-6 left-6 z-50 bg-green-500 text-black px-6 py-3 rounded-full font-bold shadow-xl flex items-center gap-2"
          >
